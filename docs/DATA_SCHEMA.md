...
**Trading 212 import:** `window.Money.importTrading212(m, holdings)` merges read-only holdings returned by `api/trading212-data.js` into `m.investments`, mutating the array in place (caller persists via `save()`). Each holding is matched against existing rows by `source === 'trading212' && ticker === <ticker>` — a match updates `shares`/`averageCost`/`currentPrice`/`contributed` in place (re-import never duplicates); no match creates a new row with `type: 'Stocks/Funds'`, `account: 'Trading 212'`, `source: 'trading212'`. `contributed` is derived as `quantity × averagePrice` (cost basis) so gain/loss stays consistent with `investmentValue()`. Manual rows (`source` unset) are never read or written by this function. The import itself is triggered from the Investments tab's "Import / Refresh Trading 212" button in `pages/money-hq.html`, which calls `GET /api/trading212-data` — a server-side proxy that requires both `TRADING212_API_KEY` and `TRADING212_API_SECRET` (Vercel env vars; `TRADING212_ENV` is optional) and always authenticates with HTTP Basic auth (`API_KEY:API_SECRET`, no raw-key fallback) when calling Trading 212's read-only `/equity/portfolio` and `/equity/account/cash` endpoints (never order/trading endpoints). If either env var is missing, the endpoint returns a safe JSON error instead of making a request. No credentials ever reach the browser. Manual refresh only — never called automatically on page load. See `SETUP.md` §7 for setup.
