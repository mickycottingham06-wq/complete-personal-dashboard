...
money: {
  ...
  investments: [ { id, name, type, ticker, shares, averageCost, currentPrice, currentValue, contributed, account, notes, source } ],  // source: ''|'trading212'
...
`window.Money.load()` / `.save(m)` follow the same upgrade-on-load pattern as every other section. `save()` also refreshes two derived caches so they can never drift from the arrays that actually own the data: each budget's `spent` (recomputed from `spending` for the current calendar month) and today's `netWorthHistory` snapshot (replaces today's entry if one already exists, otherwise appends). `window.Money.uid()` generates ids. Pure calculation reads — `netWorth()`, `totalAccounts()`, `totalAssets()`, `totalLiabilities()`, `totalInvestmentsValue()`, `totalInvestmentsContributed()`, `totalInvestmentsGain()`, `totalInvestmentsGainPct()`, `investmentValue()`, `investmentAllocation()`, `monthlyIncome()`, `monthlySpending()`, `monthlySavings()`, `savingsRate()`, `spendingByCategory()`, `incomeBySource()`, `upcomingBills()`, `monthlyRecurringTotal()`, `recentTransactions()` — never persist and never duplicate the underlying arrays. `window.Money.computeSummary()` bundles the headline numbers (net worth, monthly income/spending/savings, savings rate, total invested, investments gain/gain %, next upcoming bill) for the Command Centre preview card.

**Investments holdings model:** each `investments` row supports an optional per-holding shares/price model alongside the original manual fields, so existing manual entries keep working untouched. `ticker` (free-text symbol), `shares`, `averageCost`, `currentPrice`, and `account` (platform/pie name) are all optional — `load()` fills them in with safe defaults (`''`/`0`) on any row that doesn't have them yet, without touching a value already saved. `window.Money.investmentValue(inv)` is the single source of truth for a holding's value: it returns `shares × currentPrice` when both are set (>0), otherwise it falls back to the manually-entered `currentValue` — so `totalInvestmentsValue()` (and therefore `netWorth()` and `computeSummary()`) automatically becomes more accurate for holdings with share/price data, with zero change needed by any other reader. `window.Money.investmentAllocation(m, 'type'|'account')` groups current holding value by investment type or account/platform, same sorted `{ label, value, pct }` shape as `spendingByCategory()`.

**Trading 212 import:** `GET /api/trading212-data` is a server-side proxy requiring both `TRADING212_API_KEY` and `TRADING212_API_SECRET` (Vercel env vars; `TRADING212_ENV` is optional), always authenticating with HTTP Basic auth (`API_KEY:API_SECRET`, no raw-key fallback) against Trading 212's read-only `GET /equity/positions` (open positions) and `GET /equity/account/summary` (cash/account totals — display only, never imported as a holding) endpoints — never order/trading endpoints. If either env var is missing, the endpoint returns a safe JSON error instead of making a request. No credentials ever reach the browser. Manual refresh only — never called automatically on page load.

Trading 212 reports each position's price per share in the *instrument's own currency* (e.g. GBX pence for LSE-listed stocks, or USD/EUR for foreign listings) — multiplying that directly by quantity as if it were GBP previously inflated holding values (the cause of a bad import that added tens of thousands of pounds of fake value). The endpoint instead derives `currentValueGBP`/`costBasisGBP`/`profitLossGBP` from the position's `walletImpact` object, which Trading 212 itself has already converted into the account's home currency — those fields are `null` (never a guessed number) when `walletImpact.currency` isn't `GBP`, surfaced on the holding as `valueUnknown: true`.

`window.Money.previewTrading212(m, holdings)` is a pure read (no mutation, nothing persisted) that maps raw holdings from the endpoint into preview rows — ticker, name, quantity, instrument currency, GBP current value/cost basis/profit-or-loss (or `null` + `valueUnknown: true`), and whether the row `action` would be `add` or `update` against `m.investments`. `pages/money-hq.html`'s Investments tab uses this to render a "Confirm Trading 212 import" preview card the user must explicitly confirm (or cancel) before anything is written — no auto-apply.

`window.Money.importTrading212(m, holdings)` merges the (already-previewed, user-confirmed) holdings into `m.investments`, mutating the array in place (caller persists via `save()`). Each holding is matched against existing rows by `source === 'trading212' && ticker === <ticker>` — a match updates `shares`/`averageCost`/`currentPrice`/`currentValue`/`contributed`/`notes` in place (re-import never duplicates); no match creates a new row with `type: 'Stocks/Funds'`, `account: 'Trading 212'`, `source: 'trading212'`. `currentValue` and `contributed` (cost basis) are taken directly from the GBP-converted `currentValueGBP`/`costBasisGBP`; per-share `currentPrice`/`averageCost` are back-derived from those trusted GBP totals (never from the raw instrument-currency price) so `investmentValue()`'s shares×price fallback stays consistent with them. When a holding's value is unknown, the row's numeric fields are stored as `0` with `notes` set to a visible "value unknown — verify manually" flag rather than any guessed figure. Manual rows (`source` unset) are never read or written by this function.

`window.Money.removeTrading212Rows(m)` strips every `source === 'trading212'` row from `m.investments` (manual rows untouched) so a bad import can be cleared via the Investments tab's "Remove Trading 212 imported rows" button, which asks for confirmation before applying.

See `SETUP.md` §7 for setup.
...
- **Google Calendar** is implemented (real OAuth, read-only). `scripts/google-calendar-service.js` (`window.GoogleCalendarService`) owns the token pair (`gcal_tokens_v1` in localStorage) and calendar-specific logic separately from the generic `integrations-data.js`. `connect()` sends the browser to Google's consent screen (`calendar.events.readonly` scope only); `api/google-calendar-callback.js` exchanges the code server-side and redirects back to `pages/integrations.html` with the tokens in the URL hash, same pattern as `api/whoop-callback.js`. `refresh('googleCalendar')` calls `api/google-calendar-data.js` (a proxy to the Google Calendar API, refreshing the access token via `api/google-calendar-refresh.js` on a 401), maps the response into this shape, and writes it into `integrations.googleCalendar` — wrapped in the same try/catch that sets `status: 'Error — …'` on failure instead of throwing. `getToday(events)` / `getTomorrow(events)` / `getNext(events)` are unchanged pure local-date-aware reads used by the Integrations card, the dashboard preview, and the Daily Snapshot "Next event" chip. Env vars: `GOOGLE_CLIENT_ID` (public, also exposed to the browser via `api/config.js` as `window.GOOGLE_CLIENT_ID` to build the consent URL), `GOOGLE_CLIENT_SECRET` (server-side only, read by `api/google-calendar-callback.js` / `api/google-calendar-refresh.js`).
